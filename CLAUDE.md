...
- **ボール保持駒の予約上書きバグ修正**（`execCommand`のmove/dribble分岐冒頭）: `ball`は`ball.pieceId`で
  駒に追従する作りのため、ボール保持駒がパス/スルーパスを予約→同じ駒を選び直して「移動」に上書きした場合、
  明示的に切り離さないと移動先までボールが追従してしまう（見た目上ドリブルと区別がつかない）。
  carry=falseでこの駒がボールを保持していたら、移動前の座標で`ball={target:'cell',...}`に切り離してから
  座標を更新する。検証: node --check＋DOMスタブで`ball.target/x/y/pieceId`を直接アサート（ログ文字列だけでは
  見た目のバグを見逃す＝前回の反省点）。目視確認はブラウザ制約（claude-in-chromeはlocalhost/file://到達不可）
  のためユーザー側で実施。
