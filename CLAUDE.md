...
- オンラインバーは `URL` ボタンで `?room=...` 共有URLをコピーでき、そのURLを開くと自動参加する。ブラウザは `localStorage` にオンライン用 `clientId` を保持するため、リロード後も同じ席へ復帰しやすい。
...
- ブラウザ側は `match.turn.resolved` を受けて `TurnEvent` を再生し、最終盤面はサーバースナップショットで同期する。シュートゴール/ブロック/ミス/GKセーブ、パスカット、PK/FK、CK/GK系の代表カットインとボール軌跡はオンライン再生にも反映済み。
