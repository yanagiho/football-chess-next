...
### ⏳ 未着手・要検討（PENDING）
GDD準拠で試合の締まり・納得感に効く部分（おまかせ方針なので実装裁量あり）。※画面で目視確認しながら実装するのが安全:
4-b. **ライブのオフサイド＋位置戻し**: 現状 calcOffside は「実演ボタン」専用。実プレイのパス解決に統合し、オフサイド時は位置を戻す
4-c. **クリア処理**: 自陣のこぼれ球をDF/GKが前方へ蹴り出す
4-d. **演出を駒の上に表示**: 全画面カットインに加え、関与した駒の上に小さな効果表示
