...
### ✅ 追加で完成（2026-06 Web移植リポジトリ側で実装）
1. **ターン構成の正規化**: GDDの「前半15＋AT0〜1 → ハーフタイム → 後半15＋AT1〜3 → 試合終了」を実装。finishedフェーズ・勝敗判定・ChessClockのAT表示(45+/90+)対応
2. **シュート周りの作り込み**: ①ゴール前遮蔽で必ずブロック ②同マス敵DFのブロック抽選 ③GKセーブ時はGKが必ずキャッチ。PK=75%/FK=50%固定
3. **フリーボールの所有者決定**: ターン解決の最後に、こぼれ球を同マスの駒（複数ならコスト最大）が確保
4-a. **敵マスでの連鎖制限の厳密化**: 連鎖継続を「受け手が移動未仕込み かつ（敵マスでない or 最初のパス）」に限定
4-b. **ライブのオフサイド＋位置戻し**: オフサイドライン＝相手守備の後ろから2番目のy。前方パスをライン前で受けたら確率判定し、成立で受け手をターン開始位置へ戻し守備側ボールに
4-c. **クリア処理**: GK/DFが自陣でこぼれ球を確保したら前方(0,±)へ蹴り出す
4-d. **演出を駒の上に表示**: render()で消えないfxLayerを追加し、OFFSIDE/GOAL/BLOCK/SAVE/CLEARを関与駒の上にバッジ表示

> 未着手①〜④はすべて実装済み（2026-06）。検証は node 構文チェック＋DOMスタブ実行（ランタイムエラーなし・駒22体生成・オフサイドライン青-2/赤3を確認）。
> 残るのは**実機ブラウザでの目視チューニング**（演出の見え方・確率バランス）。これはCloudflare公開後に実URLで確認しながら微調整するのが安全。
